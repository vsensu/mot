...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 当我们渲染一个物体时要使用着色器程序
        shader.Use();
        shader.LoadUniform("light.position", camera.pos());
        shader.LoadUniform("light.direction", camera.forward());
        shader.LoadUniform("light.cutOff", glm::cos(glm::radians(12.5f)));
        shader.LoadUniform("light.outerCutOff", glm::cos(glm::radians(17.5f)));
